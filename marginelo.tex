...
The procedure consequently splits each team's rating into two mirror copies as before: one rating where the team is handicapped by $m$ points and one where it is advantaged by $m$ points, i.e.\ $m \rightarrow -m$, which we label $\mathcal{R}(m)$ and $\mathcal{R}(-m)$ respectively.
When two teams rack up a large number of combined points, the handicapped team who's points contribute with positive weight ``wins'' the game and increases its rating $\mathcal{R}(m)$ while the team with the sign flipped points ``loses'' the game and decreases it's mirrored rating $\mathcal{R}(-m)$.
...
Elo differences are always taken between the mirrored pairs $\mathcal{R}(m)$ and $\mathcal{R}(-m)$ when predicting event probabilities such that only one sign flip enters equation \eqref{win_total}. 
